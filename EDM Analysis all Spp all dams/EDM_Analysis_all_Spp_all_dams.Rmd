...
dd <- bind_rows(BON, CSL, SRKW, PDO, PDO_L1, PDO_L2, PDO_L3, PDO_L4, PDO_L5, PDO_L6)
...
for (i in 1:nrow(dd_ts)){
  ...
  simplex_fit <- simplex(ts_select$norm, lib = c(1, (length(ts_select$norm)-step_ahead)),  stats_only = T, norm_type = c("L2 norm"))
  ...
  # estimate theta
  smap_fit <- s_map(ts_select$norm, lib = c(1, (length(ts_select$norm)-step_ahead)),  stats_only = T, norm_type = c("L2 norm"), E = E_hat)
  ...
  # best fit 
  smap_best_fit <- s_map(ts_select$norm, lib = c(1, (length(ts_select$norm)-step_ahead)),  stats_only = F, norm_type = c("L2 norm"), E = E_hat, theta = theta_hat)
  
  smap_pred  <- s_map(ts_select$norm, lib = c(1, (length(ts_select$norm)-step_ahead)), pred = c( c((length(ts_select$norm)-step_ahead)),  length(ts_select$norm)), stats_only = F, norm_type = c("L2 norm"), E = E_hat, theta = theta_hat)
  
  xyy <- smap_pred["model_output"][[1]][[1]] %>% drop_na()
  plot(ts_select$year, ts_select$norm, type="l", xlab = "Time", ylab="Abundance (normalized)", main=paste(ts_select$location[[1]], ts_select$species[[1]], sep=" "))
...
# Test the causal significance of each of the species interactions in the system
...
```{R}
pdf("/Users/alisoniles/Google Drive/Coordinated Recoveries/Analysis/Coordinated-Recoveries-GitHub/EDM Analysis all Spp all dams/output/ccm_BON_Fig.pdf")
 var_names <- matrix(1, nrow=nrow(dd_ts))
 SI_embed <- matrix(ncol=nrow(dd_ts), nrow=nrow(dd_ts)) #output a matrix of significant causal interactions between variables to help determine which to use in the embeddings of each fish stock. 
 SI_rho <- matrix(ncol=nrow(dd_ts), nrow=nrow(dd_ts)) #output a matrix of correlation coefficients between variables to help determine which to use in the embeddings. 
...
for (j in 1:nrow(dd_ts)){   # the direct effect of the species in column j
  ...
  MK <- apply(rho_quant[,2:5],2,MannKendall) #This is a test for monotonic trend in a time series z[t] based on the Kendall rank correlation of z[t] and t. Here the median, maximum, and 1st & 3rd quantiles are tested, all need to be <0.05.
  FZ <- paired.r(rho_quant[1,3],rho_quant[length(libs),3], NULL, libs[1], libs[length(libs)], twotailed=FALSE) # This transforms the rho at the min library and max library length to a normally distributed Fisher's z value and then tests that the max library z is significantly higher than the begining. Independent correlations, different sample sizes. Our small sample sizes are a disadvantage here.
  CI95 <- r.con(rho_quant[length(libs),3], libs[length(libs)], p=.95, twotailed=FALSE) # Test if rho is significantly different from 0 at p=0.05 level, which is what was done for Sockey in Ye et al. 2015
  
  # Output tables for the embeddings
  SI_embed[i,j] <- (MK$`25%`[[2]]<0.01 & MK$`50%`[[2]]<0.01 & MK$`75%`[[2]]<0.01 & MK$`100%`[[2]]<0.01 & CI95[1] > 0 & (rho_quant[1,3] < rho_quant[length(libs),3])) #Criteria to include a variable in the embedding: all percentiles are increasing and the final rho is statistically greater than zero. 
  SI_rho[i,j] <- signif(rho_quant[length(libs),3],2)
 ...
 plot(rho_quant[,3]~libs,type="l",col="red",ylim=c(-0.1,1.1),lwd=2,
  ...
  #text(30,1.1,paste("100%:", signif(MK$`100%`[[2]],2), sep=" "))
  #text(30,1.05,paste("75%:", signif(MK$`75%`[[2]],2), sep=" "))
  #text(30,1,paste("50%:", signif(MK$`50%`[[2]],2), sep=" "))
  #text(30,0.95,paste("25%:", signif(MK$`25%`[[2]],2), sep=" "))
...
pdf("/Users/alisoniles/Google Drive/Coordinated Recoveries/Analysis/Coordinated-Recoveries-GitHub/EDM Analysis all Spp all dams/output/Coeff_Fig.pdf")

for (i in 1:5){ #for each of the five fish taxa
  ...
  abline(v=PDO_L, col="red", lty=2)
  
  which_var <- c(i, which(SI_embed[i,1:7]==TRUE), PDO_L) #Identifiy the significant variables to include in the analysis
  Edim <- length(which_var)
  
  Embedding <- dd_ts$data[[i]][[2]][[1]]
  d <- as.data.frame(dd_ts$data[[i]]) #select focal species data 
    for (wv in 2:length(which_var)){
    Embedding <- c(Embedding, dd_ts$data[[which_var[wv]]][[2]][[1]])
    d <- merge(d, dd_ts$data[[which_var[wv]]], by = "year", suffixes = c(Embedding[wv-1],Embedding[wv]) ) #merge the different variables together by overlapping years
    }
  year <- as.data.frame(d$year) %>% slice(2:nrow(d))
  d <- select(d, paste("norm",Embedding, sep="")) 
  ls <- nrow(d)
...
#Now, all the preceding code was just to set up the calculation. We now loop over each prediction point to actually calculate the S-map locally weighted regression for each prediction point. This calculation is quite simple. (1) We calculate the (normalized) Euclidian distance from the target point to the other points on the attractor. (2) These distances then become the weights for a weighted linear regression, which we solve using singular value decomposition (SVD).
  ...
  trange <- 1:l
  coeff <- data.frame(year, coeff)
  plot(coeff[trange,1],coeff[trange,4],type="l",col="green",xlab="time",
       ylab="interaction coefficient", lwd=2)+
