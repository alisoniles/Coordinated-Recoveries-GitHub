...
testvar <- c("flow.gageht.4","up48.apr.2", "up48.may.2", "pdo.may.2", "pdo.win.2", "up48.oct.2", "up48.nov.2", "npgo.yr.2", "hatch.spring.2", "harv_n", "pinn.CSLmales.4", "pinn.CSLbonn.4","pinn.SSLaerial.4", "SRKW.Lpod.4", "SRKW.JKLpods.5")
...
#how do variables affect recruitment of wild salmon 
            ...
            df$var = factor(df$var, levels= c("up48.apr.2", "up48.may.2", "up48.oct.2", "up48.nov.2", "pdo.may.2", "pdo.win.2",  "npgo.yr.2", "flow.gageht.4","hatch.spring.2", "harv_n", "pinn.CSLmales.4", "pinn.CSLbonn.4", "pinn.SSLaerial.4", "SRKW.Lpod.4", "SRKW.JKLpods.5"))
            
             p <- ggplot(df, aes(x=yr, y=coeff)) +
                #geom_point() +
                geom_line()+
                theme_bw() + 
                scale_color_manual(values = pal) +
                labs( subtitle = paste("Effect of VARS on", target_var, "in", mpgname,"MPG",sep = " "), x="year", y="interaction coefficient") +
               facet_wrap(~var)
          
                ggsave(filename =  paste("~/Github/Coordinated-Recoveries-GitHub/CORE EDM visual/Presentation/Various_coeff_on_" ,target_var, "in", mpgname, "MPG.pdf", sep = ""), plot = p, width = 7, height = 9.5, units = "in")  # saves the last plot
