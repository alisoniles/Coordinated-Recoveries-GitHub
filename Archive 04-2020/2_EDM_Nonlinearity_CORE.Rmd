...
#create list of beginning and end points for chunks of time series combined in the same embedding. Year must be in the first column of the time series. Complete cases only. Removes sections that are not continuous for at least cont_tp time points. Returns lib and the narrowed data block. 
...
#' T_period the period of seasonality for seasonal surrogates 
...
test_nonlin <- function(ts, method = "ebisuzaki", num_surr = 200, 
                        theta = c(0, 1e-04, 3e-04, 0.001, 0.003, 0.01, 0.03, 0.1, 0.3, 0.5, 0.75, 1, 1.5, 2, 3, 4, 6, 8, 10, 12, 16, 20, 24, 28, 32),
                        tau = tau, tp = 1, E = E, ...)
{
...
#run for either normalized or non-normalized data, and change 'variable' and names of files to save        
#vars <- c("stk", "year", "salm.rec4n", "salm.rec5n", "salm.recspn4n", "salm.recspn5n")
vars <- c("stk", "year", "rec4", "rec5", "recspn4", "recspn5") 
...
variables <- c("rec4.0", "rec5.0", "srecspn4.0", "recspn5.0")
...
save(theta_results, file = paste("Output/Rdata/2_Nonlinearity/nonlinearity_theta_test.",level[l],"_",vars[v+2],".RData", sep = ""), compress = FALSE)  
load(paste("Output/Rdata/2_Nonlinearity/nonlinearity_theta_test.",level[l],"_",vars[v+2],".RData", sep = ""))
...
saveRDS(theta_results, file = "Output/Rdata/2_Nonlinearity/Diagnostic_theta_results.RDS")
...
results<-matrix(nrow=27, ncol=11); colnames(results)<-c("stock", "MPG",  "n4", "tau4", "E4", "rho4", "n5", "tau5", "E5", "rho5", "sp_start_no") #Storing output
...
results[i,11] <- ((sp-1)*4 + 1) #record the start number of the subplot (same for recspn4 and 5)
...
#save results of stock-level diagnostic and subplot lists
save(results, file = paste("Output/Rdata/2_Nonlinearity/recspn45_stock_level_diagnostic_results.Rdata", sep=""))
save(rs4_subplot_list, file = paste("Output/Rdata/2_Nonlinearity/recspn4_stock_level_diagnostic_subplot_list.Rdata", sep=""))
save(rs5_subplot_list, file = paste("Output/Rdata/2_Nonlinearity/recspn5_stock_level_diagnostic_subplot_list.Rdata", sep=""))
```
