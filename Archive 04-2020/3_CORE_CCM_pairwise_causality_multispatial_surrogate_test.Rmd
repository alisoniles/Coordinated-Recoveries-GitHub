...
load("Data/Rdata/block_data.Rdata")  
      ...
      vars <- c("stk", "year", "recspn4", "recspn5", FP_list$FP) 
...
ETT <- read.csv("CORE_opt_E_tau_theta.csv")
```



CCM: MULTISPATIAL Test of convergence for each putative causal variable
#this version optimizes E and tau for each CCM test for the response process, following Kawatsu 2021
...
system.time({  
     ...
     level <- ETT$Level[j]
     #use data set for that level
     if (level=="ESU"){D_level <- D_ESU}
     if (level=="MFS"){D_level <- D_MFS}
     if (level=="IMN"){D_level <- D_IMN}
     if (level=="UPS"){D_level <- D_UPS}
     D_level <- as.data.frame(D_level)
     
     #load optimal theta and E for the response process
     RPn <- ETT$RP[j]
     E <- ETT$E[j]
     tau <- ETT$tau[j]
     ...
     for(i in 1:nrow(CCM)){  
     FPn <- CCM[i,1] # for each putative causal variable (forcing process) 
     D <- cbind(D_level$year, D_level[FPn],D_level[RPn])
     D <- shape_block_data(D_level[,c("year", FPn, RPn)], min_stock_size = 20)
     ...
     save(CCM, file = paste("Output/Rdata/3_CCM/multispatialCCM_optRP_",RPn,"_tau",tau,"_E",E,".Rdata", sep=""))
}#for each response process
...
for(a in (2:2)){
  ...
  for (v in 1:nrow(FP_list)){ #for each putative causal variable
    ...
    EB <- min(Emat[(Emat[,"Bn"]>=(maxrho-(maxrho*0.01))),"E"], na.rm=TRUE)
        ...
        for(j in 1:length(block)){ #for each stock
            for(tau in 1:3){ #for each tau
              ...
              out <- c(block[[j]]$stk[1],An,Bn,FP_list[v,2],FP_list[v,3],FP_list[v,4],EB,tau,CCMboot$rho[length(CCMboot$rho)],CCM_sig_test)
