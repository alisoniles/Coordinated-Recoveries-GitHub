...
compute_nonlinearity_aggregated <- function(block_data, name)
{
    ...
    simplex_output <- simplex(x, lib = lib, pred = lib, E = 2:8, exclusion_radius = 0, silent = TRUE)
...
test_nonlinearity_aggregated <- function(block_data, name, num_shuffles = 500)
{
    get_smap_stats <- function(x, lib, E)
    {
        if(is.null(E))
        {
            ...
            simplex_output <- simplex(x, E = 2:8, exclusion_radius = 0, silent = TRUE)
            best_rho_E <- simplex_output$E[which.max(simplex_output$rho)]
            best_mae_E <- simplex_output$E[which.min(simplex_output$mae)]
            best_E <- min(best_rho_E, best_mae_E)
        }
        
        # compute theta using s-map and E 
        smap_output <- s_map(x, lib = lib, pred = lib, E = best_E, exclusion_radius = 0, silent = TRUE)
        
        best_rho <- max(smap_output$rho)
        best_mae <- min(smap_output$mae)
        df <- data.frame(delta_mae = best_mae - smap_output$mae[smap_output$theta == 0], best_E)
        return(df)
    }
    ...
    # null distribution. #shuffle the data but leave the NAs where they are because they generally line up with the beginning of the time series.
    ...
    null_dist <- do.call(rbind, lapply(1:num_shuffles, function(i) {
        ...
        for(i in 1:length(block_data)) {
            temp <- replace(ag_ret_n[[i]], is.finite(ag_ret_n[[i]]), sample(ag_ret_n[[i]][is.finite(ag_ret_n[[i]])])) 
            returns_shuffle <- c(returns_shuffle, temp)
            }
