...
for(a in (1)){
  An_list <-  c("salm.recspn4n.0")
  An <- An_list[a]
  An_short_list <- c("recspn4")
...
save(CCM_summary, file = paste("Output/Rdata/3_CCM/multispatial/3_multispatial_results_", level, "_and_smaller_clusters_", An_short, ".Rdata", sep=""))
...
for(a in (1:2)){
  ...
  for (v in 1:length(var_list)){ #for each putative causal variable
    Bn <- var_list[v,1]
...
rm(CCM_4, CCM_5, CCM_summary, ESU, MFS, IMN, UPS, level)
...
library(sunburstR)
    ...
    # data <- data %>%
    #     filter(level != "") %>%
    #     mutate(path = paste(level,cat,subcat, sep="-")) %>%
