...
# Save smap coefficients
     ...
     if (k>1){smap_coeffs_ESU_mean <- cbind(smap_coeffs_ESU_mean,  k_stock_mean_coeff[,2:ncol(embeddings_list)])}
     }
...
target_vars <- c("salm.rec_n.0", "salm.rec3_n.0", "salm.rec4_n.0", "salm.rec5_n.0")
...
for(u in c(0)){ #} c(0,1,3,5)){
    ...
    D <- separate(D, model, into=c("model","target","offset"), sep = "_")  #add columns for model number, target variable
    D$offset <- NULL
...
# For each variable, plot the changing coefficients over time from different mutivariate embeddings
            ...
            p1 <- ggplot(df, aes(x=year, y=IS)) +
              geom_line(aes(col=causal.subcat)) +    
            ...
            p2 <- p1 + facet_grid(causal.offset ~ salmoncohortlabels) 
            
            print(p2)
            
            ggsave(filename = paste("Output/Figures/5_MVE/5_MVE_", varcat[i], "_interaction_coeff_", mpgname, ".pdf", sep = ""), plot = p, width = 7, height = 9.5, units = "in")  # saves the last plot
