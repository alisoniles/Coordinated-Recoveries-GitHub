...
for(tv in 1:length(target_vars)){ # for each target variable
  load(file = paste("Output/Rdata/5_MVE/5_MVE_work_space_",name,"_",target_vars[tv],".RData", sep = "")) 
...
for(u in c(0)){ #} c(0,1,3,5)){
    ...
    mpgname <- "Snake River ESU"
        for(tv in 1:length(target_vars)){ # for each target variable 
        smap_coeffs_ESU_mean <- readRDS(file = paste("Output/Rdata/5_MVE/5_MVE_coeffs_ESU_mean_",name,"_",target_vars[tv],".RData", sep = ""))
