...
for(u in c(1,3,4,5)){#)){ #,3,4,5)){# #For each MPG except Lower Snake
    ...
    target_var <- c("rec4_n", "rec5_n")
        for(tv in 1:2){
          ...
          #remove either fourth year (if rec_5), or fifth year (if rec4) river variables
          a <- c(4,5) 
...
# Multiview embedding - make multivariate embeddings of all possible combinations of the chosen variables from the CCM analysis. Always include the target variable and the number of effective spawners in each embedding. 
          
          chosen_vars <- ccm$target
          num_vars <- length(chosen_vars) #number of predictor variables
          ...
          E <- 3
          lagged_block <- make_block(MVE_block[, 2:NCOL(MVE_block)], max_lag = max_lag)#, max_lag = max_lag,t = MVE_block[, 1], lib = lib, tau = tau,restrict_to_lib = FALSE)
          ...
          #The original code that excludes embeddings without at least one coordinate with 0 time lag isn't working properly because when max-lag is set one and all variables are 0 lag, it returns that none of the embeddings are valid but they all should be. I'm going to ignore this because the number of effective spawners with zero lag will be included in all embeddings. 

# make in-sample forecasts
     in_results <- block_lnlp(lagged_block, lib = lib, pred = lib, method = "simplex", 
                              columns = embeddings_list, 
                              target_column = target_var[tv],
                              stats_only = TRUE, first_column_time = TRUE)
     
     # rank embeddings
     num_embeddings <- NROW(in_results)
     k <- floor(sqrt(num_embeddings))
     k <- as.numeric(k)
     in_sample_ranking <- order(in_results$rho, decreasing = TRUE)
     ranked_embeddings <- embeddings_list[in_sample_ranking,]
     best_embeddings <- ranked_embeddings[1:k,]
