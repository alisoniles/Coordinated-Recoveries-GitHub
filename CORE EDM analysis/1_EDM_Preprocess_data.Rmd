...
make_block <- function(stock_df, env_data, esu_data)
    {
        ...
        # offset hatchery data from 2 to 5 years after brood year - assuming the hatchery releases age 2 smolts and they compete with the naturally spawned smolts starting at age 2
        for(i in 2:5) { 
              desired_years <- stock_df$year + i
        ...
        # offset pinniped data from 2 to 5 years after brood year - assuming sea lions eat smolts and returns
        for(i in 2:5) { 
              desired_years <- stock_df$year + i
              index_in_spp_data <- match(desired_years, esu_data$year)
              index_in_stock_df <- 1:length(desired_years)
        ...
        # offset CA sealion pup data from 2 to 5 years after brood year plus 3 years as only 3+ year old sea lion males swim up to Oregon
        for(i in 5:8) { 
              desired_years <- stock_df$year + i
              index_in_spp_data <- match(desired_years, esu_data$year)
              index_in_stock_df <- 1:length(desired_years)
        ...
        # line up orca data
        for(i in 3:5) { # offset orca data from 3 to 5 years after brood year - assuming orcas just eat the returning fish
              desired_years <- stock_df$year + i
              index_in_spp_data <- match(desired_years, esu_data$year)
              index_in_stock_df <- 1:length(desired_years)
