    ...
    normalize_by_cycle_line <- function(ts)
{
     ...
     preprocess_stock <- function(stock_df)
    {
        ...
        stock_df$harv_n <- temp$ts #normalized harvest
        ...
        stock_df$harv_sigma <- temp$sigma
              ...
              index_in_stock_df <- 1:length(desired_years)
              df <- data.frame(matrix(NA, nrow = length(desired_years), ncol = 1))
              df[1] <- stock_df$harv_n[index_in_spp_data]
              df.colnames <- paste("salm.harv_n", i, sep = ".")
              stock_df[, df.colnames] <- df
              }
...
make_block <- function(stock_df, env_data, esu_data)
    {
        ...
        pinn_names <- c("pinn.CSLbonn", "pinn.CSLhaulout", "pinn.CSLmales", "pinn.CSLmales6", "pinn.HSaerial", "pinn.HSbonn", "pinn.SSLaerial", "pinn.SSLbonn", "pinn.SSLrookORraw", "pinn.TOTbonn")
        ...
        # offset CA sealion pup data from 2 to 5 years after brood year. Add 6 years to each as only 6+ year old sea lion males swim up to Oregon and eat adult salmon (ref in Chasco paper)
        for(i in c(8:11)) { 
              desired_years <- stock_df$year + i
              index_in_spp_data <- match(desired_years, esu_data$year)
              index_in_stock_df <- 1:length(desired_years)
              ...
              df.colnames <- paste("pinn.CSLpup", (i-6), sep = ".")
              stock_df[, df.colnames] <- df
        }
...
make_block <- function(stock_df, env_data, esu_data)
    {
        ...
        for(i in 0:0) { 
              ...
              df <- data.frame(matrix(NA, nrow = length(desired_years), ncol = NCOL(CSL_pups_data)))
              df[index_in_stock_df,] <- CSL_pups_data[index_in_spp_data ]
              df.colnames <- paste("pinn.CSLpup", (i), sep = ".")
              stock_df[, df.colnames] <- df
        }
        ...
        # line up orca data
        for(i in 0:0) { # offset orca data from 4 to 5 years after brood year - assuming orcas just eat the returning fish
              desired_years <- stock_df$year + i
              index_in_spp_data <- match(desired_years, esu_data$year)
              index_in_stock_df <- 1:length(desired_years)
