...
SRKW_data <- read.csv("Data/csv_data/SRKW.csv")
...
make_block <- function(stock_df, env_data, sealion_data, SRKW_data)
    {
        ...
        sealion_names <- c("Spp.Int_SL.pups", "Spp.Int_SL.males")
        sealion <- normalize(sealion_data[, sealion_names])
        SRKW_names <- "Spp.Int_SRKW"
        SRKW <- normalize(SRKW_data[, SRKW_names]) 
        
        # line up environmental data
        for(i in 0:6) { # lag environmental data from 0 to 6 years after brood year. 
        ...
        for(i in 0:6) { # lag sealion data from 0 to 6 years after brood year. 
        desired_years <- stock_df$yr + i
        index_in_spp_data <- match(desired_years, sealion_data$Year)
        index_in_stock_df <- 1:length(desired_years)
        sl_cols <- data.frame(matrix(NA, nrow = length(desired_years), ncol = NCOL(sealion)))
        sl_cols[index_in_stock_df,] <- sealion[index_in_spp_data, ]
        colnames <- paste(sealion_names, i, sep = "_")
        stock_df[, colnames] <- sl_cols
        }
        
        # line up orca data
        for(i in 0:6) { # lag orca data from 0 to 6 years after brood year. 
        desired_years <- stock_df$yr + i
