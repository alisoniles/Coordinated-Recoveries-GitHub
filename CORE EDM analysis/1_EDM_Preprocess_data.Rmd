    ...
    normalize_by_cycle_line <- function(ts)
{
     ...
     preprocess_stock <- function(stock_df)
    {
        ...
        stock_df$salm.eff_n.0 <- temp$ts #normalized number of effective spawners
        #stock_df$eff_mu <- temp$mu
        #stock_df$eff_sigma <- temp$sigma
        
        temp <- normalize_by_cycle_line(stock_df$rec3)
        stock_df$salm.rec3_n.0 <- temp$ts #normalized 3 year old recruits
        ...
        #stock_df$rec3_sigma <- temp$sigma
              ...
              df[1] <- stock_df$salm.rec3_n.0[index_in_spp_data]
              df.colnames <- paste("salm.rec3_n", i, sep = ".")
              stock_df[, df.colnames] <- df
              }
        temp <- normalize_by_cycle_line(stock_df$rec4)
        stock_df$salm.rec4_n.0 <- temp$ts #normalized 4 year old recruits
        ...
        #stock_df$rec4_sigma <- temp$sigma
              ...
              index_in_stock_df <- 1:length(desired_years)
              df <- data.frame(matrix(NA, nrow = length(desired_years), ncol = 1))
              df[1] <- stock_df$salm.rec4_n.0[index_in_spp_data]
              df.colnames <- paste("salm.rec4_n", i, sep = ".")
              stock_df[, df.colnames] <- df
              }

        temp <- normalize_by_cycle_line(stock_df$rec5)
        stock_df$salm.rec5_n.0 <- temp$ts #normalized 5 year old recruits
        ...
        #stock_df$rec5_sigma <- temp$sigma
              ...
              index_in_stock_df <- 1:length(desired_years)
              df <- data.frame(matrix(NA, nrow = length(desired_years), ncol = 1))
              df[1] <- stock_df$salm.rec5_n.0[index_in_spp_data]
              df.colnames <- paste("salm.rec5_n", i, sep = ".")
              stock_df[, df.colnames] <- df
              }
        
        temp <- normalize_by_cycle_line(stock_df$rec)
        stock_df$salm.rec_n.0 <- temp$ts #normalized total recruits
        #stock_df$rec_mu <- temp$mu
        #stock_df$rec_sigma <- temp$sigma
        
        #temp <- normalize_by_cycle_line(stock_df$ret)
        #stock_df$salm.ret_n <- temp$ts #normalized returns
...
make_block <- function(stock_df, env_data, esu_data)
    {
        ...
        # offset harvest data from 3 to 5 years after brood year - assuming the harvest is only affecting returns 3+
        for(i in c(3:5)) { 
              desired_years <- stock_df$year + i
              index_in_spp_data <- match(desired_years, esu_data$year)
              index_in_stock_df <- 1:length(desired_years)
...
mydetrend <- function(block)
{
  if(NCOL(block)==1)
  {
         index <- complete.cases(block)
         x <- as.matrix(block[index])
         dx <- detrend(x)
         block[index] <- dx
  }
  else
  for(i in 1:NCOL(block)) 
    {
         index <- complete.cases(block[,i])
         x <- as.matrix(block[index,i])
         dx <- detrend(x)
         block[index,i] <- dx
    }
     return(block)
    ...
    preprocess_stock <- function(stock_df)
    {
        ...
        # Normalize on a 1 year cycle line (i.e. NO cycle line!)
        temp <- mydetrend(stock_df$spawn_tot)
        temp <- normalize(temp)
        stock_df$salm.eff_n <- temp #normalized number of effective spawners
      
        temp <- mydetrend(stock_df$rec3)
        temp <- normalize(temp)
        stock_df$salm.rec3_n <- temp #normalized 3 year old recruits

        temp <- mydetrend(stock_df$rec4)
        temp <- normalize(temp)
        stock_df$salm.rec4_n <- temp #normalized 4 year old recruits

        temp <- mydetrend(stock_df$rec5)
        temp <- normalize(temp)
        stock_df$salm.rec5_n <- temp #normalized 5 year old recruits

        temp <- mydetrend(stock_df$rec)
        temp <- normalize(temp)
        stock_df$salm.rec_n <- temp #normalized total recruits
        
        temp <- mydetrend(stock_df$harvest)
        temp <- normalize(temp)
        stock_df$harv.SPSS <- temp #normalized harvest with name in the same format as ESU level harvest data
        
        temp <- mydetrend(stock_df$recspn)
        temp <- normalize(temp)
        stock_df$salm.recspn_n <- temp #normalized total recruits
        
        temp <- mydetrend(stock_df$recspn3)
        temp <- normalize(temp)
        stock_df$salm.recspn3_n <- temp #normalized 3 year old recruits

        temp <- mydetrend(stock_df$recspn4)
        temp <- normalize(temp)
        stock_df$salm.recspn4_n <- temp #normalized 4 year old recruits

        temp <- mydetrend(stock_df$recspn5)
        temp <- normalize(temp)
        stock_df$salm.recspn5_n <- temp #normalized 5 year old recruits
...
env_data <- read.csv("Data/csv_data/SRSS_env_prelim.csv")
esu_data <- read.csv("Data/csv_data/CORE_2020_data_ESU_NAforZeros.csv")
mpg_data <- read.csv("Data/csv_data/CORE_2020_data_MPG.csv")
...
make_block <- function(stock_df, env_data, esu)
    {
        ...
        esu_data[, 2:NCOL(esu_data)] <- mydetrend(esu_data[, 2:NCOL(esu_data)])
        ...
        mpg_tempdata$mpg <- NULL
        mpgdata_names <- c("flow.mean", "flow.peak", "flow.gageht", "flow.peakday")
        mpg_tempdata[, 2:NCOL(mpg_tempdata)] <- mydetrend(mpg_tempdata[, mpgdata_names]) 
        mpg_tempdata[, 2:NCOL(mpg_tempdata)] <- normalize(mpg_tempdata[, mpgdata_names]) 
