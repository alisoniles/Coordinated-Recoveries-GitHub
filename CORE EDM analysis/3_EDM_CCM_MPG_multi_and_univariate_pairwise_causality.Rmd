...
for(c in c(1:length(varcat))) {
  ...
  p1 <- ggplot(D_plot, aes(x=offset, y=rho_eff)) +
    ...
    labs(title= varcat[c], subtitle = "Multivariate CCM with block_lnlp function, without effective spawners in the embedding ", x="Variable offset, year", y=expression(paste("Cross-mapping correlation, ", {rho}))) + 
...
ggsave(filename = paste("Output/Figures/3_CCM/3_CCM_",varcat[c],"_multivariate_block_lnlp.pdf", sep = ""), plot = p2, width = 7, height = 9.5, units = "in")  
