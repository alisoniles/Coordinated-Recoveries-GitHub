...
D_ESU <- rbind(stock_data$'Bear Valley Creek'[vars],  #1, MFS
              ...
              stock_data$'Lostine River'[vars],  #10, IMN
...
dist_mat <- matrix(data = NA, nrow=22, ncol = 22)
for(i in c(2:23)){
  for(j in c(2:23)){
...
#Clean up dist_mat
mpg <- c("MFS"  , "MFS"  , "MFS"  , "MFS"  , "MFS"  , "MFS"  , "IMN"  , "IMN"  , "IMN"  , "IMN"  , "IMN"  , "IMN"  , "IMN"  , "UPS"  , "UPS"  , "UPS"  , "UPS"  , "UPS"  , "UPS"  , "SFS"  , "SFS"  , "SFS")
colnames(dist_mat) <- paste(names(D)[2:23], ", ", mpg, sep="")
rownames(dist_mat) <- paste(names(D)[2:23], ", ", mpg, sep="")
...
png(file = paste("Output/Figures/2_Nonlinearity/simplex_dendrogram_IMN_", tarvar, ".png", sep=""),
width=2000, height=1400)
