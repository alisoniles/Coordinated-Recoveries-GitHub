    ...
    if(CCM$library[i]==library_vars[l]){
      ...
      twin.sur <- ParSurCI(x, E, y, block.sur, lib = lib, lib.parms = libsize)
         ...
         ccm.tmp <- cbind(ccm.tmp, twin.sur$rho)
           ...
           CCM$d_rho[i] = rev(ccm.tmp$rho)[1] - ccm.tmp$rho[1] #delta rho - the change in rho from smalles to largest library size
           CCM$u_rho[i] = rev(twin.sur$rho$u_rho)[1] #difference between rho and the top of the 95% CI of the surrogate data rho, for the largest library size
...
for(c in c(1:length(varcat))) {
  ...
  p1 <- ggplot(D_plot, aes(x=offset, y=u_rho)) +
    ...
    labs(title= D_plot$catlabels[1], subtitle = "CCM Twin Surrogate significance test: uses the ccm function; without effective spawners in the embedding ", x="Variable offset, year", y="difference between true {rho} and the 95% CI of the surrogate {rho}") + 
    scale_color_manual(values = mcc[,1], name="Time series", breaks = mcc[,2], labels = mcc[,3]) +
    scale_shape_manual(name="Convergence test, {delta} rho>0",  values=c("FALSE"=1, "TRUE"=16),  breaks = c("TRUE", "FALSE"), labels = c("True", "False"))
