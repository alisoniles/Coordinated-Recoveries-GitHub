...
values <-  rbind("jan"='#800000', "feb"='#e6194b', "mar"='#f58231', "apr"='#ffe119', "may"='#bfef45', "jun"='#3cb44b', "jul"='#469990', "aug"='#42d4f4', "sep"='#4363d8', "oct"='#000075', "nov"='#911eb4', "dec"='#f032e6',  "sti"='#99CC33', "fti"='#CC6600', "lusi"='#00FFCC', "tumi"='#006633', "tdmi"='#CC6699',  "win"='#CC6699', "spr"='#99CC33', "sum"='#00FFCC', "fal"='#CC6600', "yr"='#999999', "yravg"='#999999', "yrsum"='#333333', "mean"='#469990', "peak"='#42d4f4', "gageht"='#4363d8', "peakday"='#000075')
breaks  <- rbind("jan", "feb", "mar", "apr", "may", "jun", "jul", "aug", "sep", "oct", "nov", "dec", "sti", "fti", "lusi", "tumi", "tdmi", "win", "spr", "sum", "fal", "yr", "yravg", "yrsum", "mean", "peak", "gageht", "peakday")
labels <-  rbind("Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec", "STI", "FTI", "LUSI", "TUMI", "TDMI", "Winter", "Spring", "Summer", "Fall", "Year", "Year Avg", "Year sum", "River mean flow", "River peak flow", "River gage height", "River peak day")
scm <- cbind(values,breaks,labels)
colnames(scm) <- c("values","breaks","labels")
...
for(c in c(1:length(varcat))) {
  ...
  print(unique(D_plot$subcat))
  p1 <- ggplot(D_plot, aes(x=offset, y=rho_eff)) +
    geom_line(aes(col=subcat)) +
    geom_point(aes(col=subcat, shape=(MannKendall==TRUE))) +
    ...
    labs(title="Convergent cross-mapping", subtitle = paste("Variable category '",varcat[c],"'", sep = ""), x="Variable offset, year", y=expression(paste("Cross-mapping correlation, ", {rho}))) +  
    scale_color_manual(values = scm[,1], name="Time series", breaks = scm[,2], labels = scm[,3]) +
...
#ggsave(filename = paste("Output/Figures/3_CCM/3_CCM_",varcat[c],".pdf", sep = ""), plot = p2, width = 7, height = 9.5, units = "in")  
...
#---------------------------------------------------------------------------------------------------------  
# Multivariate CCM, including the number of effective spawners in the embedding 
            lnlp_block_eff <- as.matrix(cbind(library_block[,1:dim(library_block)[2]-1], block["eff_n"], block[target_var]))
            rownames(lnlp_block_eff) <- NULL #remove rownames to supress error in CCM: NAs introduced by coersion
            CCMlibrarynames <- as.character(cbind(t(CCMlibrarynames),"eff_n") )
...
phys_d <- d[d$cat=="flow" | d$cat=="npgo" | d$cat=="pdo" | d$cat=="upw" | d$cat=="arc", ]
biol_d <- d[d$cat=="hatch"  | d$cat=="harv" | d$cat=="csl" | d$cat=="ssl" | d$cat=="hseal" | d$cat=="orca", ]
...
values <-  rbind("spring"='#800000', "summer"='#e6194b', "total"='#f58231', "bonn"='#ffe119', "CSLhaulout"='#bfef45', "CSLmales"='#3cb44b', "HSaerial"='#469990', "HSbonn"='#42d4f4', "SSLaerial"='#000075', "SSLbonn"='#911eb4', "SSLrookORraw"='#f032e6', "TOTbonn"='#800000', "CSLpup"='#e6194b',  "CPUE"='#f58231', "ssb"='#ffe119', "Jpod"='#bfef45',"Kpod"='#3cb44b', "Lpod"='#469990', "KLpods"='#42d4f4', "JKLpods"='#4363d8', "JKLdeaths"='#000075', "JKLbirths"='#911eb4')
...
p1 <- ggplot(phys_d$cat=="arc", aes(x=tp, y=rho)) +
