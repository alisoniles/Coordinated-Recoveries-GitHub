...
mpg <- c() 
  for(stk in 1:length(block_data)) { 
    ...
    data <- block_data[c(mpg==1 | mpg==3 | mpg==5)] #only include Imnaha, Middle Fork Salmon and Upper Salmon MPGs. We tossed the Lower Snake and South Fork Salmon because there weren't enough stocks for these MPGs
    ...
    minlib <- lib[,2]-lib[,1]
    lib <- lib[minlib>5,]
    libsize <- c(50,100,150,300,450,700,1000,NROW(block))  #round(seq(NROW(block)/8, NROW(block), by = NROW(block)/8))
...
D$region <- factor(D$mpg, levels=c("Snake River ESU", "Imnaha", "Middle Fork Salmon", "Upper Salmon"), labels=c("Snake River ESU", "Imnaha", "Middle Fork Salmon", "Upper Salmon"))

D$salmoncohort <- factor(D$library, levels=c("salm.rec_n.0","salm.rec3_n.0", "salm.rec4_n.0", "salm.rec5_n.0"), labels=c("All recruits", "3 yr old recruits", "4 yr old recruits", "5 yr old recruits"))
...
p2 <- p1 + facet_grid(region ~ salmoncohort) 
