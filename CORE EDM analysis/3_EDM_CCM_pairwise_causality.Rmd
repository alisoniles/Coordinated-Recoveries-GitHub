    ...
    ccm_table <- do.call(rbind, lapply(block_data, function(stock_df) 
        ...
        ccm_rhos <- do.call(cbind, lapply(env_names, function(env_var) {
            output <- block_lnlp(block, tp = 0, target_column = env_var, 
                                 columns = c("rec4_n", "eff_n"), silent = TRUE)
            return(output$rho)
        }))
