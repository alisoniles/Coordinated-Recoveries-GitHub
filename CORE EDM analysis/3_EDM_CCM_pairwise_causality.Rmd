...
ccm <-  ccm(ccm_block, lib = lib, E = as.numeric(as.character(CCM_outputlist$E[i])), lib_column = library_var, target_column = target_var, first_column_time = TRUE, tp = 0, lib_sizes = libsize, random_libs = FALSE, num_samples=100, replace=FALSE, RNGseed=2301, silent = TRUE)      
...
The first analysis uses the block_lnlp function on a delay coordinate embedding of the library variable to predict the target variable. The embedding dimension used is the optimal E for the library variable (our variable of interest). The number of effective spawners could be included in this analysis as one of the dimensions, however I am currenly running this as a univariate analysis. 
...
for(u in c(1,3,4,5)){
  data <- block_data[c(mpg==u)]
  mpgname <- as.character(data[[1]]$mpg[1])
  CCM_outputlist<- readRDS(paste("Output/Rdata/3_CCM/univariate/3_CCM_MPG_",mpgname,".RDS", sep = ""))
...
load("Data/Rdata/block_data_no_lags.Rdata")  
    ...
    for(i in c(1:NROW(CCM_outputlist)) ){  
      ...
      #*********** block_lnlp function with effective spawners included***********
