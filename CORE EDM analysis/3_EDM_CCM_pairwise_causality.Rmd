...
saveRDS(CCM_outputlist, file = paste("Output/Rdata/3_CCM_multivar_spawn_rec_",name,".RDS", sep = ""), compress = FALSE)
...
p1 <- ggplot(d, aes(x=offset, y=rho)) +
    ...
    theme_bw() + 
    labs(title=paste(name, "MPG", sep=" "), subtitle = "Multivariate CCM with spawners included in the attractor reconstruction", x="Environmental variable offset, year", y=expression(paste("Cross-mapping correlation, ", {rho}))) +  
    scale_color_manual(values = c("jan"='#800000', "feb"='#e6194b', "mar"='#f58231', "apr"='#ffe119', "may"='#bfef45', "jun"='#3cb44b', "jul"='#469990', "aug"='#42d4f4', "sep"='#4363d8', "oct"='#000075', "nov"='#911eb4', "dec"='#f032e6', "yr"='#a9a9a9', "win"='#a9a9a9'), name="Time series", breaks = c("jan", "feb", "mar", "apr", "may", "jun", "jul", "aug", "sep", "oct", "nov", "dec", "yr", "win"), labels = c("Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec", "NPGO year", "PDO window")) +
...
ggsave(filename = paste("Output/Figures/3_CCM_multivar_spawn_rec_",name,".pdf", sep = ""), plot = p2, width = 7, height = 9.5, units = "in")  # saves the last plot

  }
 
```

#Univariate CCM between all stock variables and all environmental variables
```{R}
load("Data/Rdata/block_data.Rdata")  
