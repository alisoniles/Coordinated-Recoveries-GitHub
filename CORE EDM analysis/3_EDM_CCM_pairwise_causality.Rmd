...
#Multivariate CCM for the major population groups
...
mpg <- c() 
  ...
  for(u in c(1:5)){
    ...
    CCM_outputlist <- expand.grid(env_vars, lags, stringsAsFactors = FALSE) 
    CCM_outputlist[,3] <- paste(CCM_outputlist[,1], CCM_outputlist[,2], sep = "_")
    CCM_outputlist[,4:6] <- matrix(NA, nrow = NROW(CCM_outputlist), ncol = 6)
    colnames( CCM_outputlist) <- c("env_fac","offset","target","target E","rho","95p_crit")

    #Perform cross mappings:      
    for(i in c(1:NROW(CCM_outputlist)) ){  
    
      target_var <- CCM_outputlist[i,3]
      env_factor <- CCM_outputlist[i,1]
          ...
          # determine the best univariate embedding dimension and theta for the target variables - use time series of environemtnal vars from 1957 to 2009+7(lags)=2016 for ALL lags, so all lags of the same environmental variable have the same E
            ...
            CCM_outputlist[i,4] <- simplex_output$E[which.max(simplex_output$rho)] #Apply optimal E that maximizes rho to the following CCM...
          ...
          # CCM from library var to target var   
            ...
            CCM_outputlist[i,6] <- qnorm(0.95, sd = 1/sqrt(xmap$num_pred - 3)) #qnorm is used to look up percentiles of the standard normal distribution. The 0.95 quantile is the 95th percentile. qnorm produces the boundary value that the rho needs to be greater than. 
    }
    
saveRDS(CCM_outputlist, file = paste("Output/Rdata/block_lnlp ",name,".RDS", sep = ""), compress = FALSE)
...
d <-  d[order(d$env_fac),]
d[,7] <- paste('Spawner*recruit xmap', d$env_fac)
colnames(d)[7] <- "xmap"
...
p1 <- ggplot(d, aes(x=offset, y=rho)) +
    ...
    theme_bw() + 
    labs(title=paste(name, "MPG", sep=" "), x="Offset", y="Rho") +  
...
p2 <- p1 + facet_grid(env_fac ~ month)  

print(p2)

ggsave(filename = paste("Output/Figures/3_block_lnlp_",name,".pdf", sep = ""), plot = p2, width = 7, height = 9.5, units = "in")  # saves the last plot

  }
 
```

#Pairwise CCM between all stock variables and all environmental variables
```{R}
load("Data/Rdata/block_data.Rdata")  
