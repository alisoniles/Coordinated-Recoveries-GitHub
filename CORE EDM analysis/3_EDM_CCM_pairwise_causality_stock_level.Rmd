...
block <- list(block_data$'Bear Valley Creek'[vars4],  
              ...
              block_data$'Wenaha River'[vars4],
              block_data$'Yankee Fork'[vars4],
              block_data$'Bear Valley Creek'[vars5],  
              block_data$'Big Creek'[vars5],
              block_data$'Grande Ronde Upper Mainstem'[vars5],
              block_data$'Imnaha River'[vars5],
              block_data$'Lemhi River'[vars5],
              block_data$'Loon Creek'[vars5],
              block_data$'Marsh Creek'[vars5],
              block_data$'Salmon River Lower Mainstem below Redfish Lake'[vars5],
              block_data$'Salmon River Upper Mainstem above Redfish Lake'[vars5],
              block_data$'Sulphur Creek'[vars5], 
              block_data$'Valley Creek'[vars5],
              block_data$'Wallowa River, Hurricane Creek, Bear Creek, and Lostine Rivers'[vars5],
        ...
        for(j in 1:length(block)){ #for each stock
          ...
          data <- block[[j]] 
            ...
            for(tau in 1:5){ #test a range of tau
              ...
              #We now determine the proper embedding dimension for cross-mapping. Following the procedure of Deyle et al. (2016) ee determine E that gives optimal prediction of cross-mapping lagged 1 time step.
...
load(file = paste("Output/Rdata/3_CCM/stock_level/3_stock_results_recspn45.Rdata", sep=""))
...
#Narrow results to those interactions with significant convergence
    CCM <- CCM[CCM$pval<=0.05,]
    
    CCMt <- CCM %>% #only include the tau for each variable that optimizes rho
      group_by(stock, response_var, RP, causal_var, cat, subcat, offset, E) %>%
      mutate(best_tau = tau[rho==max(rho, na.rm = TRUE)])
    CCMt <- CCMt[CCMt$tau==CCMt$best_tau,]
    
    CCMt <- CCMt %>% #only include the offset for each variable that optimizes rho 
...
save(CCM, file = paste("Output/Rdata/3_CCM/stock_level/3_stock_CCM_twinsur_recspn45.Rdata", sep=""))
```
...
#plot as treemaps
  ...
  dflist <- list(Stock4Spr, Stock5Spr)
    for (i in 1:2){
      data <- dflist[[i]]
    p <- treemap(data,
                index=c("cat", "subcat"),
                ...
                palette = "-RdYlBu",
                bg.labels=c("white"),
                title="Stock level CCM of 4yo recruits per spawner: terminal rho",
                align.labels=list(
                  c("center", "top"), 
                  c("left", "bottom")
                )  
        ...
        Stock5Spr <- CCM[CCM$RP=="4yo" & CCM$runtype=="Spring",]
          stock <- Stock5Spr$stock
          cat <- Stock5Spr$cat
          subcat <- Stock5Spr$subcat
          value <- Stock5Spr$ter_rho
    data <- data.frame(stock,cat,subcat,value)
    ...
    p <- treemap(data,
                ...
                palette = "-RdYlBu",
                bg.labels=c("white"),
                title="Stock level CCM of 5yo recruits per spawner: terminal rho",
                align.labels=list(
                  c("center", "top"), 
                  c("left", "bottom")
                )  
