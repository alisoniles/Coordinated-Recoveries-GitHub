...
The time-delay of the embedding, tau, is introduced to improve the observability of a time series (Judd and Mees 1998). The time lag is chosen to optimize the spread of the embedded time series without confusing the dynamics. 
...
For all stocks, the optimal tau was <= 5. Run the rest of the diagnostic plots for tau=1:5. Many stocks had missing data points, which the mutual information calculation cannot handle properly. To get better estimates of Tau for each stock, consider running on the largest continuous time chunks or doing two separate evaluations for a stock with a missing year in the middle of the time series. 
...
The maximum E that can be tested depends on the length of the data as max E < sqrt(n). 
For each stock, we evaluated a range of embedding dimensions (E from 1:7) using simplex projection and predicting 1:12 years into the future. We also varied tau. Forecast skill (rho) between predicted and observed values determines the optimal model parameters.
...
D_ESU <- as.matrix(rbind(stock_data$'Bear Valley Creek'[vars], NA,  
              ...
              stock_data$'Lostine Creek'[vars], NA,  
...
variables <- c("salm.rec4n", "salm.rec5n", "salm.recspn4n", "salm.recspn5n")
results <- matrix(nrow=16, ncol=4); colnames(results)<-c("level", "variable", "tau", "E") #Matrix for storing output
results[,2] <- variables

l <- 1; v <- 1
for(l in (1:4)){  #loop through each analysis level the ESU and the different MPGs
...
maxE<-20 #Maximum E to test based on the square root of the number of samples, also depends on tau

Emat<-matrix(nrow=maxE-1, ncol=4); colnames(Emat)<-c("E", "Tau2", "Tau3", "Tau4") #Matrix for storing output
...
Edf <- pivot_longer(Edf, cols=c(2:4))
Edf$chosen <- 0
Edf$chosen [(Edf$E==E2 & Edf$name=="Tau2") | (Edf$E==E3 & Edf$name=="Tau3") | (Edf$E==E4 & Edf$name=="Tau4")] <- 1
...
maxTP <- 20 #Maximum TP to test

TPdf <- matrix(nrow=maxTP, ncol=4); colnames(TPdf)<-c("TP", "Tau2", "Tau3", "Tau4") #Matrix for storing output
TPdf[,"TP"] <- c(1:maxTP) 
...
TPdf <- pivot_longer(TPdf, cols=c(2:4))
...
```{R}
# Find the optimal tau for each stock

rm(list=ls()) 
load("Data/Rdata/stock_data_no_lags.Rdata")  

tarvars <- c("salm.recspn4n", "salm.recspn5n") 

Tau_df<-matrix(nrow=28, ncol=6); colnames(Tau_df)<-c("stock", "MPG",  "n", "No. missing years", "rec4n tau", "rec5n tau") #Storing output

i <- 1
for(i in 1:28){
par(mfrow = c(1, 2))
  stock <- names(stock_data)[i]
    Tau_df[i,1] <- stock
    Tau_df[i,2] <- stock_data[[i]]$mpg[1]
    
  A4 <- stock_data[[i]]$salm.rec4n
  A5 <- stock_data[[i]]$salm.rec5n

  year <- stock_data[[i]]$year
  year4 <- year[complete.cases(A4)]
  year5 <- year[complete.cases(A5)]
  
  A4 <- A4[complete.cases(A4)]
  A5 <- A5[complete.cases(A5)]
  
  Tau_df[i,3] <- length(A5)
  Tau_df[i,4] <- sum(diff(year5)!=1)
  
ami4 <- mutual(A4, lag.max = 20, plot=TRUE, main=paste(stock, ", rec4n", sep=""))
  ami4min <-min(which(diff(ami4)>=0))-1
  Tau_df[i,5] <- ami4min
  points(ami4min, ami4[ami4min+1][[1]], cex = 1, col = "red")

ami5 <- mutual(A5, lag.max = 20, plot=TRUE, main=paste(stock, ", rec5n", sep=""))
  ami5min <-min(which(diff(ami5)>=0))-1
  Tau_df[i,6] <- ami5min
  points(ami5min, ami5[ami5min+1][[1]], cex = 1, col = "red")
...
subplot_list[[(4+((-tau-1)*4))]] = pTP
 ...
 n <- length(subplot_list)
 nCol <- 4 #floor(sqrt(n))
 pDIAGNOSTICS <- do.call("grid.arrange", c(subplot_list, ncol=nCol, top=paste("lag plots for ", level[l], sep = "")))
ggsave(filename = paste("Output/Figures/2_Nonlinearity/",level[l],"_diagnostic_plots_recspn45.pdf", sep = ""), plot = pDIAGNOSTICS, width = 9, height = 9, units = "in")  # saves the last plot
