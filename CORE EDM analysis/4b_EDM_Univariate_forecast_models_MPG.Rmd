...
simplex(composite_ts[, c("Year", var)], E = 2:8, lib = composite_lib, pred = composite_pred)
})
...
smap_out <- lapply(names(composite_ts)[2:6], function(var) {
  s_map(composite_ts[, c("Year", var)], E = best_E[var], lib = comp_lib, 
        pred = comp_pred)
})
...
### LEFT OFF HERE
# Next time to visualize pred vs obs

simplex_data <- lapply(names(composite_ts)[2:6], function(var) {
simplex(composite_ts[, c("Year", var)], E = 2:6, lib = comp_lib, pred = comp_pred, stats_only = FALSE)
})
...
model_output <- univ_output[[1]]$model_output 
