...
compute_ccm <- function(data, name)
{
    ...
    {
        lib[i,] <- c(last+1, last + length(data[[i]][[1]])) 
        last <- lib[i,2]
    }
    ...
    #concatenate the time series of each variable together
        ...
        valid <- is.finite(merged_data$rec5_n) & is.finite(merged_data$eff_n)
        block <- stock_df[valid,]
     
        env_names <- c(names(data[[1]][28:77]))

    
    ccm_table <- do.call(rbind, lapply(data, function(stock_df) 
         {
