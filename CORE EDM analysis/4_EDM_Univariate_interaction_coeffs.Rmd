...
if(tarvar=="salm.rec4n.0") {vars <-  c("year", tarvar,"salm.effn.0", "salm.rec5n.0", "salm.rec3n.0", var_names[,1])} 
if(tarvar=="salm.rec5n.0") {vars <-  c("year", tarvar,"salm.effn.0", "salm.rec4n.0", "salm.rec3n.0", var_names[,1])}
...
IC <- readRDS("Output/Rdata/4_IC/base embed with effn and same cohort returning in different years/Interaction_coeffs_ESU_rec5.RDS") 
...
for(c in c(1:length(varcat))) {
  ...
  p1b <- ggplot(D, aes(x=subcat)) +
    ...
    # overlay the model rho info, transformed to match roughly the range of the IC
                ...
                geom_hline(aes(yintercept=0.51/5), colour="black", linetype="dashed") +
  ...
  p <- grid.arrange(p1a, p1b, layout_matrix=rbind(c(1,2)), top=paste("Partial effects of ",varcat[c], " on ", tarvar, " at chosen offsets from brood year", sep=""), left="Interaction coefficient")
...
IC_ESU <- readRDS(paste("Output/Rdata/4_IC/base embed with effn and same cohort returning in different years/Interaction_coeffs_ESU_", tarvarshort, ".RDS", sep="")) 
IC_MFS <- readRDS(paste("Output/Rdata/4_IC/base embed with effn and same cohort returning in different years/Interaction_coeffs_MiddleForkSalmon_", tarvarshort, ".RDS", sep="")) 
IC_IMN <- readRDS(paste("Output/Rdata/4_IC/base embed with effn and same cohort returning in different years/Interaction_coeffs_Imnaha_", tarvarshort, ".RDS", sep="")) 
IC_UPS <- readRDS(paste("Output/Rdata/4_IC/base embed with effn and same cohort returning in different years/Interaction_coeffs_UpperSalmon_", tarvarshort, ".RDS", sep="")) 
...
for(c in c(1:length(varcat))) {
  ...
  p <- ggplot(D, aes(x=subcat)) +
    ...
    # overlay the model rho info, transformed to match roughly the range of the IC
                geom_point(shape=23, size = 3, aes(y=rho, colour = subcat)) + 
                scale_y_continuous(name = "IC axis", sec.axis = sec_axis(~.*1, breaks = c(0, 0.2, 0.4, 0.6, 0.8), labels = c("0","0.2", "0.4", "0.6", "0.8"), name = "model forecast skill, rho"))+ # now adding the secondary axis and, very important, reverting the above transformation
                scale_colour_manual(values = mcc[,1], breaks = mcc[,2], labels = mcc[,3])  +
                geom_hline(aes(yintercept=0.51), colour="black", linetype="dashed") +
