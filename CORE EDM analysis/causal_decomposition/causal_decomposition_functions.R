# 'r' is the only parameter involved in the causal decomposition analysis. 
# The strategy of selecting r is to maximize the separability while
# maintaining the orthogonality of the IMFs, thereby avoiding spurious 
# causal detection resulting from poor separation of a given signal. 
# Yang et al. calculate the nonorthogonal leakeage and the root-mean-square(RMS) 
# of the pairwise  correlations of the IMF for each r with an increment of 0.05 
# in the uniform spacce between 0.05 and 1. A general guideline for selecting
# r is to minimize the RMS of the pairwise correlations of the IMFs (ideally 
# under 0.05) while maintaining the nonorthogonal leakage also under 0.05.

# nensemble: the number of ensemble to average the results of noise-assisted
# empirical mode decomposition. The number of ensemble affects the error 
# (or degree of distortion by added noise) of IMFs. The degree of distortion 
# can be reduced by a large number of trails and is estimated as noise level 
# dived by the square root of the number of ensemble. The degreee of distortion 
# is suggested to be less than 0.05 in the ensemble EMD.

set_r_noise <- function(s1){
     
     # normalize time series
     s1 <- s1-mean(s1)
     s1 <- s1/std(s1)
     
     # set range of r values to test
     rnoiseseq <- seq(from=0.05, to=1, by=0.05)
     
     #initiate results vector
     x <- matrix(nrow=length(rnoiseseq),ncol=4)
     
     r <- 5
     for(r in 1:length(rnoiseseq)){
          rnoise <- rnoiseseq[r]
          nensemble <- (rnoise/0.01)^2
          imfs <- eemd(s1, noise_strength = rnoise, ensemble_size = nensemble, num_siftings = 0, S_number = 10, num_imfs=4)
          cormat <- cor(imfs[,1:(ncol(imfs)-1)])
          cormat <- cormat[upper.tri(cormat)]
          rms <- sqrt(mean(cormat^2)) #calculate the root-mean-squared error of the correlations between the IMFs
          
          #check orthogonality
          OI <- matrix(nrow=ncol(imfs), ncol=ncol(imfs))
          for(j in 1:(ncol(imfs))){
               for(k in 1:(ncol(imfs))){
               t <- (imfs[,j]*imfs[,k])/(s1^2)
               t <- t[is.finite(t)]
               OI[j,k] <- sum(t, na.rm=TRUE) 
          }}
          
          OI2 <- matrix(nrow=ncol(imfs), ncol=ncol(imfs))
          for(j in 1:(ncol(imfs))){
               for(k in 1:(ncol(imfs))){
                    t <- (imfs[,j]*imfs[,k])/(imfs[,j]^2+imfs[,k]^2)
                    t <- t[is.finite(t)]
                    OI2[j,k] <- sum(t, na.rm=TRUE) 
               }}
          OI2 <- sum(OI2[upper.tri(OI2)])
          
          x[r,] <- c(rnoise, nensemble, rms, OI2)
     }
     colnames(x) <- c("r", "NE", "RMS", "OI")
     return(x)
}
...
causal_decomposition <- function (s1,s2,rnoise,nensemble){
     ...
     # Complete Ensemble Empirical Mode Decomposition with Adaptive Noise algorithm, a variant of ceemdan.
...
phasefcimf <- function(imfs1,imfs2,imfsize){
     psi <- matrix(nrow=imfsize,ncol=1)
     for (i in 1:imfsize){
          h1 <- hilbert(imfs1[,i])
          h2 <- hilbert(imfs2[,i])
          a1 <- angle(h1)
          a2 <- angle(h2)
          phasediff <- unwrap(a1[1:length(a1)])-unwrap(a2[1:length(a2)])
          psi[i] <- pscoherence(phasediff)
     }
     return(psi)
}
...
nvar <- function(imf){
     v <- diag(var(imf)) # calculate variance
     nv <- v/sum(v) # calculate normalized variance
     return(nv)
}


#********************************************************************************************
pscoherence <- function(phasediff){
     
     # transform phase difference to a complex plane with radius of one
     z <- exp((phasediff)*sqrt(as.complex(-1)))
